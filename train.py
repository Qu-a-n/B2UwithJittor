...
def resume_state(load_path, optimizer, scheduler) -> Tuple[int, optim.Adam, optim.lr_scheduler.MultiStepLR]:
...
if __name__ == "__main__":
    ...
    # BEGIN Training ---------------------------------
    for epoch in range(epoch_bg if epoch_bg == 1 else epoch_bg + 1, epochs + 1):
        cnt = 0
        model.train()
        epoch_loss_all = []
        ...
        for i, clean in enumerate(trainLoader):
            ...
            logger.info(
                    '{:04d} {:05d} diff={:.6f}, exp_diff={:.6f}, Loss_Reg={:.6f}, Lambda={:.6f}, Loss_Rev={:.6f}, Loss_All={:.6f}, Time={:.4f}'
            .format(epoch, i, torch.mean(diff**2).item(), torch.mean(exp_diff**2).item(),
            ...
            for valid_name, valid_images in valid_dict.items():
                ...
                for t in range(repeat_times):
                    for idx, img in enumerate(valid_images):
                        ...
                        if i == 0 and epoch == config.n_snapshot:
